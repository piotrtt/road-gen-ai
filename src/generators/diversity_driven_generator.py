"""
Diversity-driven road network generator.

Samples N candidate networks from a base generator (random or
least-generated) and keeps the one that is most diverse from the
networks already produced, using the project's combined similarity
metric (Levenshtein on type sequences + normalized parameter distance).

No LLM is involved — see `hybrid_generator.py` for the LLM+metric
combination that earns the "hybrid" label.
"""
...
class DiversityDrivenGenerator(BaseGenerator):
    """
    ...
    def generate(self, num_components: int) -> Dict:
        ...
        existing = self.storage.load_all(approach="diversity_driven")
    ...
    def get_statistics(self) -> Dict:
        """Get statistics about diversity-driven generation."""
        networks = self.storage.load_all(approach="diversity_driven")
...
if __name__ == "__main__":
    # Test diversity-driven generator
    print("Testing DiversityDrivenGenerator...")
    print("=" * 60)
